...
export function RelatedDiscoverPosts({ posts, title = '相关笔记' }: RelatedPostsProps) {
    ...
    <section className="mb-8">
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4">
        {posts.map((post, i) => {
          ...
          const authorName = post.profiles?.display_name || post.author_name || '匿名';
          return (
            <Link
              key={post.id}
 ...
 */
export function DiscoverPostsSection({ posts, title = '社区笔记' }: RelatedPostsProps) {
  if (!posts.length) return null;
