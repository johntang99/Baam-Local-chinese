...
const suggestedTags = [
  '宝藏小店', '法拉盛', '新移民美食', '平价美食', '中餐推荐',
  '探店分享', '省钱攻略', '纽约租房', '华人医生', '驾照攻略',
];
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6">
      ...
      <div className="flex flex-wrap gap-2 px-3.5 py-2.5 border border-gray-200 rounded-xl min-h-[44px] items-center focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/10 transition cursor-text">
        ...
        {tags.length < maxTags && (
          <input
            ...
            placeholder={tags.length === 0 ? '输入话题标签...' : ''}
      ...
      {availableSuggestions.length > 0 && tags.length < maxTags && (
        <div className="mt-3">
          <p className="text-xs text-gray-400 mb-2">推荐话题</p>
      ...
      <p className="text-xs text-gray-400 mt-2">最多 {maxTags} 个标签，按回车或逗号添加</p>
    </div>
  );
}
