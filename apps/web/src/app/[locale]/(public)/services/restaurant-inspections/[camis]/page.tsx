...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${displayName} 卫生评分${latest?.grade ? ` ${latest.grade}级` : ''} | Health Inspection · Baam`,
    description: `${displayName}（${address}）的纽约市卫生检查评分${latest ? `：${latest.grade}级（${latest.score}分）` : ''}。查看完整检查历史和违规记录。`,
    openGraph: {
      title: `${restaurant.dba} — 卫生评分`,
