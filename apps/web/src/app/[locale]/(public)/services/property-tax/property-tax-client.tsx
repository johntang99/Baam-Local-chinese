  ...
  return (
    <div>
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-5 sm:p-6 mb-6">
        <h2 className="text-base font-bold text-gray-900 mb-4">{convert('查询房产税')}</h2>
        <p className="text-xs text-gray-500 mb-4">{convert('输入房产地址，查看评估值、税额和交易历史。NYC以外地区可加城市名精确搜索（如 "23 Main St, Middletown"）')}</p>
      ...
      {/* Error */}
      {error && <div className="bg-red-50 border border-red-200 text-red-700 rounded-xl p-4 mb-6 text-sm">{convert(error)}</div>}

      {/* Results */}
      {results !== null && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-bold text-gray-900">{convert('查询结果')}</h2>
            <span className="text-sm text-gray-400">{convert(`找到 ${results.length} 处房产`)}</span>
          </div>

          {results.length === 0 ? (
            <div className="bg-gray-50 rounded-2xl p-8 text-center">
              <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl">🏠</div>
              <h3 className="text-base font-bold text-gray-700 mb-1">{convert('未找到房产')}</h3>
              <p className="text-sm text-gray-500 mb-2">{convert('请检查地址拼写，确保使用英文地址（如 123 Main Street）')}</p>
              <p className="text-xs text-gray-400">{convert('提示：某些房产可能以不同地址格式登记（如联合地址"86-92"），或登记在不同的城镇名下。尝试只输入门牌号不加城市名，或使用相邻地址搜索。')}</p>
            </div>
          ) : (
                  ...
                  <Wrapper key={r.bbl || i} {...wrapperProps}
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{r.boro} {r.owner ? `· ${convert('业主')}: ${r.owner}` : ''}</p>
                    ...
                    <div className={cn('grid gap-4 text-center', r.estimatedTax > 0 ? 'grid-cols-3' : 'grid-cols-2')}>
                      <div>
                        ...
                        <div className="text-[11px] text-gray-500">{convert('评估值')}</div>
                      </div>
                      <div>
                        <div className="text-base font-bold text-blue-600">{formatMoney(r.marketValue)}</div>
                        <div className="text-[11px] text-gray-500">{convert('市场估值')}</div>
                      </div>
                      {r.estimatedTax > 0 && (
                        <div>
                          <div className="text-base font-bold text-primary">{formatMoney(r.estimatedTax)}{convert('/年')}</div>
                          <div className="text-[11px] text-gray-500">{convert('预估税额')}</div>
                        </div>
                      )}
                    </div>
                  </Wrapper>
