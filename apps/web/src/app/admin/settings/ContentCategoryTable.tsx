...
export function ContentCategoryTable({
  ...
  const handleDelete = async (cat: AnyRow) => {
    if (!confirm(`确定要删除分类「${cat.name_zh || cat.name_en || cat.slug}」吗？`)) return;
    const fd = new FormData();
    fd.set('table_name', tableName);
      ...
      <div className="overflow-x-auto">
        <table className="data-table w-full">
          ...
          <tbody>
            {categories.length === 0 ? (
              <tr>
                <td colSpan={7} className="text-center text-text-muted py-8">暂无分类</td>
              </tr>
            ) : (
              categories.map((cat) => (
                <tr key={cat.id}>
                  ...
                  <td>
                    <div className="flex items-center gap-3">
                      <button onClick={() => openEdit(cat)} className="text-xs text-primary hover:underline">编辑</button>
                      <button onClick={() => void handleDelete(cat)} className="text-xs text-red-600 hover:underline">删除</button>
                    </div>
                  </td>
                </tr>
              ))
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100]" onClick={closeEdit}>
          <div className="bg-white rounded-xl p-6 w-[520px] max-w-[90vw]" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1">中文名</label>
                  <input
                    value={form.name_zh}
