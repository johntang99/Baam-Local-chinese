...
const profileTypeBadge: Record<string, { cls: string; label: string }> = {
  user: { cls: 'badge badge-gray', label: '普通用户' },
  creator: { cls: 'badge badge-green', label: '创作者' },
  kol: { cls: 'badge badge-purple', label: 'KOL' },
  expert: { cls: 'badge badge-blue', label: '专家' },
  influencer: { cls: 'badge badge-yellow', label: '达人' },
};
...
export default function VoicesTable({ tab, profiles, applicationProfiles, featuredProfiles, siteParams }: VoicesTableProps) {
      ...
      <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
        ...
        {applicationProfiles.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-text-muted">暂无待审核的申请</p>
          </div>
        ) : (
    ...
    return (
      <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
        ...
        {featuredProfiles.length === 0 ? (
          <div className="p-12 text-center">
            <p className="text-text-muted">暂无Featured达人</p>
          </div>
        ) : (
          <div className="p-5">
  ...
  return (
    <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
      ...
      {profiles.length === 0 ? (
        <div className="p-12 text-center">
          <p className="text-text-muted">该站点暂无达人</p>
        </div>
      ) : (
        <table className="data-table">
          <thead>
