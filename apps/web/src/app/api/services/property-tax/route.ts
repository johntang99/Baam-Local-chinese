...
function normalizeAddress(addr: string): string {
  ...
  // Replace ordinal words/suffixes with numbers
  for (const [word, num] of Object.entries(ordinals)) {
    normalized = normalized.replace(new RegExp(`\\b${word}\\b`, 'g'), num);
  }
  // AVENUE → AVE, STREET → ST, BOULEVARD → BLVD, etc. (NYC uses full words)
  // Actually NYC uses full words like "AVENUE", so keep as-is
  return normalized;
}
...
export async function GET(request: Request) {
  ...
  // NYS parcel IDs contain dots/dashes (e.g., "21-10.-1"), NYC BBLs are 10-digit numbers
  ...
  if (isNYSParcel && bbl) {
    ...
    return handleNYSDetail(bbl, county, headers);
  }
  ...
  try {
    ...
    if (bbl) {
      ...
      // Fetch from new assessment dataset + PLUTO in parallel
      ...
      // Fallback to old dataset if new one has no data
      if (assessments.length === 0) {
        ...
        if (oldRes.ok) {
          ...
          // Map old fields to match new dataset shape
      ...
      // New dataset uses cur* prefix for current values
      ...
      // Build address from new dataset fields
      ...
      // Assessment history (multiple years, deduplicated by year)
      ...
      // Try to fetch sale history from ACRIS
    ...
    // Normalize address (FIFTH → 5, 5TH → 5, etc.)
    const normalizedAddr = normalizeAddress(address!);

    // Parse house number and street from the input address
    const addrMatch = normalizedAddr.match(/^(\d[\d-]*)\s+(.+)$/);
    const houseNumFull = addrMatch ? addrMatch[1] : ''; // "36-40" or "350"
    const streetName = addrMatch ? addrMatch[2] : normalizedAddr;

    // Strip city/state/zip if user entered full address (e.g., "36-40 Main St, Flushing, NY 11354")
    const cleanStreet = streetName.replace(/,.*$/, '').trim();

    // Try new dataset (FY2023-2027): multiple address format attempts
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let raw: any[] = [];
    ...
    // Deduplicate by BBL, keep latest year
...
async function handleNYSQuery(address: string, county: string, fetchHeaders: Record<string, string>) {
  ...
  // Extract municipality if user typed "90 North St, Middletown" or "90 North St, Middletown, NY 10940"
  let municipality = '';
  const commaParts = streetRaw.split(',').map((s: string) => s.trim());
  if (commaParts.length >= 2) {
    // First part = street, second part = city/municipality
    const cityPart = commaParts[1];
    // Skip if it looks like a state abbreviation (2 letters) or zip code
    if (cityPart.length > 2 && !/^\d{5}/.test(cityPart) && !/^[A-Z]{2}$/.test(cityPart)) {
      municipality = cityPart.charAt(0).toUpperCase() + cityPart.slice(1).toLowerCase();
    }
  }
  const streetName = commaParts[0].trim(); // street without city/state/zip

  // Split street into name + suffix (e.g., "MAIN STREET" → street="Main", suff="St")
  // NYS dataset stores them separately: parcel_address_street + parcel_address_suff
  ...
  // County name needs title case for NYS API
  const countyName = county.split(' ').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ');

  try {
    // Search NYS dataset using $where for case-insensitive matching
    // NYS data has inconsistent casing (some UPPERCASE, some Title Case)
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let raw: any[] = res.ok ? await res.json() : [];

    // Broader fallback: try without suffix separation (some records store "MAIN ST" as one field)
    if (raw.length === 0) {
      const fullStreet = streetName.toUpperCase();
      const fallbackMuniFilter = municipality ? ` AND municipality_name='${municipality}'` : '';
      const fallbackWhere = `county_name='${countyName}' AND parcel_address_number='${houseNum}' AND upper(parcel_address_street) like '%${fullStreet.split(' ')[0]}%'${fallbackMuniFilter}`;
      const fallbackParams = new URLSearchParams({ $where: fallbackWhere, $order: 'roll_year DESC', $limit: '20' });
      const res2 = await fetch(`${NYS_ASSESSMENT_URL}?${fallbackParams}`, { headers: fetchHeaders });
      raw = res2.ok ? await res2.json() : [];
    }

    // Last resort: use NYS GIS geocoder → parcel lookup → assessment roll
    // Handles combined addresses (e.g., "71-73 East Main St" when user searches "71 East Main St")
    if (raw.length === 0) {
      const fullAddress = `${houseNum} ${streetName}`;
      const parcel = await geocodeToParcel(fullAddress, municipality || '', 'NY', '');
      if (parcel?.printKey) {
        // Found parcel via geocoding — now get assessment data using print_key_code
    ...
    const properties = Array.from(byParcel.values()).map((r) => {
      ...
      return {
        bbl: r.print_key_code || '', // NYS uses print_key_code instead of BBL
        address: addr.trim(),
        ...
        assessedTotal: assessed,
        marketValue: market,
        estimatedTax: 0, // NYS tax rates vary by municipality, not calculated
        year: r.roll_year || '',
        // NYS-specific fields
...
// Fetch NYS municipal tax rates by SWIS code
...
async function handleNYSDetail(parcelId: string, county: string, fetchHeaders: Record<string, string>) {
  try {
    // Query by print_key_code — may need county to narrow down
    const params = new URLSearchParams({
    ...
    // Fetch tax rates for this municipality
    ...
    // NYS tax rates can be per $1,000 of assessed value OR full value
    // The valueType field tells us which
    ...
    // Equalization rate (assessed / market)
